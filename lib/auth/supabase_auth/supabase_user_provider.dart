...
class CruappSupabaseUser extends BaseAuthUser {
  CruappSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> cruappSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CruappSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
